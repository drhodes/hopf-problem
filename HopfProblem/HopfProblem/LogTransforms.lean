...
/-! ### Section 5.3 & 7.1: Multiple Fibre Fundamental Groups and Boundary Meridians -/

/-- The presentation of the boundary collar fundamental group π₁(M_j):
    extension 1 → π₁(U_∂) → π₁(M_j) → ℤ/m_j → 1,
    generated by the lattice Λ, the deck generator ĝ_j, and the boundary circle meridian σ. -/
structure BoundaryCollarPi1 (m : ℕ) where
  meridian_order : ℕ := m
  meridian_order_pos : meridian_order ≥ 2

/-- The meridian element σ in π₁(M_j) satisfies σ⁻¹ = ĝ_j^{m_j} t_{-v_j}.
    The kernel of π₁(M_j) → π₁(N_j) is the normal closure of σ. -/
def collar_meridian_relation (m : ℕ) (hm : m ≥ 2) : BoundaryCollarPi1 m where
  meridian_order := m
  meridian_order_pos := hm

/-- For m₁ = 3, the boundary collar meridian order is 3. -/
theorem collar_meridian_N1 : (collar_meridian_relation 3 (by decide)).meridian_order = 3 := rfl

/-- For m₂ = 4, the boundary collar meridian order is 4. -/
theorem collar_meridian_N2 : (collar_meridian_relation 4 (by decide)).meridian_order = 4 := rfl

/-- Quotient presentation: killing the meridian σ enforces ĝ_j^{m_j} = t_{v_j},
    yielding the fundamental group of the reduced bielliptic fibre π₁(S_j) = Λ ⋊_{A_j} ℤ_{m_j}. -/
theorem bielliptic_pi1_presentation_order (m : ℕ) (hm : m ≥ 2) : m > 1 := by
  omega

/-- The normal bundle torsion order:
    the normal bundle O_X(S_j)|_{S_j} has exact order m_j in Pic(S_j).
    Since m₁ = 3 and m₂ = 4, neither normal bundle is trivial (both orders > 1). -/
theorem normal_bundle_order_N1 : m1 = 3 ∧ m1 > 1 := by
  decide

theorem normal_bundle_order_N2 : m2 = 4 ∧ m2 > 1 := by
  decide

/-- Coprimality of the order pair (m₁, m₂) = (3, 4): gcd(3, 4) = 1. -/
theorem multiple_fibre_orders_coprime : Nat.gcd m1 m2 = 1 := by
  decide
