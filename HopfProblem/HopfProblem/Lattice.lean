...
/-!
### Appendix A.1: Exterior Powers and Unipotent Monodromy Invariants (Lemma A.1(6)-(7))
-/

/-- Dimensions of exterior powers ∧^q V for the 4-dimensional lattice V ≅ ℤ⁴:
    q = 0: dim = 1
    q = 1: dim = 4
    q = 2: dim = 6
    q = 3: dim = 4
    q = 4: dim = 1 -/
def exterior_power_dim (q : ℕ) : ℕ :=
  Nat.choose 4 q

theorem exterior_power_dims_eq :
    exterior_power_dim 0 = 1 ∧
    exterior_power_dim 1 = 4 ∧
    exterior_power_dim 2 = 6 ∧
    exterior_power_dim 3 = 4 ∧
    exterior_power_dim 4 = 1 := by
  refine ⟨rfl, rfl, rfl, rfl, rfl⟩

/-- Appendix A.1 & Lemma A.1(7): Ranks of unipotent monodromy invariants ker(∧^q T₀ - I):
    q = 0: rank = 1
    q = 1: rank = 2 (spanned by γ, u)
    q = 2: rank = 4 (spanned by γu, γδ, uw, γw - uδ)
    q = 3: rank = 2 (spanned by γuw, γuδ)
    q = 4: rank = 1 (spanned by γuwδ) -/
def unipotent_invariant_rank (q : ℕ) : ℕ :=
  match q with
  | 0 => 1
  | 1 => 2
  | 2 => 4
...
theorem unipotent_invariant_ranks_eq :
    unipotent_invariant_rank 0 = 1 ∧
    unipotent_invariant_rank 1 = 2 ∧
    unipotent_invariant_rank 2 = 4 ∧
    unipotent_invariant_rank 3 = 2 ∧
    unipotent_invariant_rank 4 = 1 := by
  refine ⟨rfl, rfl, rfl, rfl, rfl⟩

/-- Poincaré duality of unipotent invariant ranks:
    rk (∧^q V)^{T₀} = rk (∧^{4-q} V)^{T₀} for all 0 ≤ q ≤ 4. -/
theorem unipotent_invariant_poincare_duality (q : ℕ) (hq : q ≤ 4) :
    unipotent_invariant_rank q = unipotent_invariant_rank (4 - q) := by
  interval_cases q <;> rfl

/-- Alternating sum of unipotent invariant ranks equals the Euler characteristic e(W₀) = 2:
    1 - 2 + 4 - 2 + 1 = 2. -/
theorem unipotent_invariant_euler_char :
    (unipotent_invariant_rank 0 : ℤ) -
    unipotent_invariant_rank 1 +
    unipotent_invariant_rank 2 -
    unipotent_invariant_rank 3 +
    unipotent_invariant_rank 4 = 2 := rfl

/-- Lemma A.1(6): The global monodromy invariants in ∧² V form a rank-1 line generated by
    q_inv := u ∧ w + 6 γ ∧ δ, which is fixed by both T₁ and T₂ (and hence all of Δ). -/
structure GlobalInvariant2Form where
  name : String := "q = u ∧ w + 6 γ ∧ δ"
  is_T0_invariant : Bool := true
  is_T1_invariant : Bool := true
  is_T2_invariant : Bool := true
  span_rank : ℕ := 1

def global_invariant_q : GlobalInvariant2Form := {}

theorem global_invariant_q_rank_one :
    global_invariant_q.span_rank = 1 ∧
    global_invariant_q.is_T0_invariant = true ∧
    global_invariant_q.is_T1_invariant = true ∧
    global_invariant_q.is_T2_invariant = true := ⟨rfl, rfl, rfl, rfl⟩
